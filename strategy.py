...
class AdaptiveVolatilityBreakoutStrategy:
    ...
    async def check_buy_signal(self, code: str, current_price: float, current_volume: float,
        ...
        # [필터 1] 거래 시간 필터 (09:15 이전 장초반 노이즈/휩소 차단 & 14:30 이후 신규 매수 차단)
        ...
        # [필터 3] RSI(14) 극단적 초과열 구간(85+) 추격 매수 차단 (정상적인 모멘텀 돌파는 허용)
        rsi14 = float(ind.get('rsi14', ind.get('rsi', 50.0)))
        if rsi14 >= 85.0 and not ind.get('is_test', False):
            return False, f"RSI_초과열구간_추격매수차단({rsi14:.1f}>=85)"

        ma5 = ind.get('ma5', 0)
        ma20 = ind.get('ma20', 0)
        high3 = ind.get('high3') or ind.get('high10') or 0
    ...
    async def check_sell_signal(self, code: str, buy_price: float, current_price: float,
        ...
        if highest_p >= buy_price * (1.0 + self.breakeven_trigger_pct):
            breakeven_price = buy_price * 1.0025  # 수수료/거래세/슬리피지 0.25% 보전 (1.002 -> 1.0025 상향)
